...
func _unhandled_input(event):
	if ((triggered_count < 2 or (triggered_count > 2 and triggered_count <= 6))
	...
	(triggered_count == 7 and (event is InputEventMouseButton and\
	 event.button_index == MOUSE_BUTTON_RIGHT and event.is_pressed())) or\
	(triggered_count == 8 and event.is_action_pressed("reset_screen") ):
		triggered_count += 1
